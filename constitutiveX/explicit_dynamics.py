...
class CDMPlaneStrainX:
    ...
    def stress_update(self, h):
        ...
        if self.nonlocal_var is not None:
           input_list[self.nonlocal_var.Q_nonlocal] = self.nonlocal_var.get_quadrature_values()
    ...
    # @profile
    def step(self, h):
        del_t_mid = (h + self.del_t)/2. if self.del_t is not None else h
        self.del_t = h
...
class ImplicitNonlocalVariable(NonlocalInterface):
    ...
    def __init__(
        ...
        A_form = (ufl.inner(test_function,trial_function) + self.l**2. * ufl.inner(ufl.grad(test_function),ufl.grad(trial_function))) * self.quadrature_rule.dx
