...
> NewtonSolverStrain<'a, N, STRESS_STRAIN, N_PARAMETERS, PARAMETERS, KAPPA, T>
{
    ...
    pub fn solve(
        ...
        loop {
            ...
            //this part of the code is ugly because we copy the matrix into a dynamically sized one
            let mut dres_dyn = DMatrix::<f64>::zeros(N, N);
            dres_dyn.copy_from(&dres);
            ...
            loop {
                ...
                // stagnates.
                //TODO: kappa.min()<0.0 is only useful if it is the accumulated plastic strain
