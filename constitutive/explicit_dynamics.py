...
# class CDMInterface:
    ...
    # def __init__(
        # self, V, u0, v0, t0, f_ext, bcs, M, damping_factor=None, bc_mesh = "current"
    # ):
...
class CDM:
    def __init__(
        self, V, u0, v0, t0, f_ext, bcs, M, law, stress_rate = None, damping_factor=None, calculate_F = False, bc_mesh = "current"
    ):
        self.QT = helper.quadrature_tensor_space(V, shape=(3, 3))
        self.QV = helper.quadrature_vector_space(V, dim=6)
...
# class CDM2:
    ...
    # def stress_update(self, h):
        ...
        # temp_stress = self.stress.vector().get_local()
        # temp_stress = self.stress_rate(temp_stress, h * 0.5)

        # eps = cpp.strain_increment(self.L.vector().get_local(),h)
        
        # self.ip_loop.set(cpp.Q.SIGMA, temp_stress)
        # self.ip_loop.set(cpp.Q.EPS, eps)
