...
def test_uniaxial_stress():
    ...
    assert norm(diff, description.dxm) < 1e-10 / youngs_modulus * 0.01
    ...
    assert norm(diff, description.dxm) < 1e-10 / youngs_modulus * 0.02
...
def test_uniaxial_strain():
    ...
    assert norm(diff, description.dxm) < 1e-10 / analytical_stress
    ...
    description_3d = IncrSmallStrainProblemDescription(
        laws=wrapped_1d_law,
        displacement_field=u_3d,
        quadrature_degree=1,
    )
    description_3d.add_boundary_conditions(bc_left, bc_right)
    problem_3d = description_3d.to_problem()
    ...
    assert norm(diff, description_3d.dxm) < 1e-10 / analytical_stress
    ...
    assert norm(diff, description_3d.dxm) < 1e-10 / norm(
        problem.stress_0, description.dxm
    )
    ...
    assert norm(diff, description_3d.dxm) < 1e-14 / norm(problem._u, description.dxm)
...
def test_plane_strain():
    ...
    assert norm(problem.stress_0[2], description.dxm) > 1e-2
    ...
    problem_3d.update()
    # test that the stress is nonzero in 33 direction
    assert norm(problem_3d.stress_0[2], description.dxm) > 1e-2

    # test that the displacement is the same in both versions
    diff = problem_3d._u - problem._u
    assert norm(diff, description.dxm) / norm(problem._u, description.dxm) < 1e-14
    # test that the stresses are the same in both versions
    diff = problem_3d.stress_0 - problem.stress_0
    assert norm(diff, description.dxm) / norm(problem.stress_0, description.dxm) < 1e-10
...
def test_plane_stress():
    ...
    assert norm(problem.stress_0[2], description.dxm) < 1e-10
...
def test_3d():
    ...
    diff = u_fenics - u
    assert norm(diff, description.dxm) < 1e-8 / norm(u, description.dxm)
