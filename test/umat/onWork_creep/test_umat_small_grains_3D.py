...
class TestUniaxial(unittest.TestCase):
    def test_3d_small_grains(self):
        ...
        bcs =[bcBottom, bcOrigin, bcPointX]
        ...
        CreepLoad = 50.     # creep at 50 MPa
        top_displ = []
        
        V0 = FunctionSpace(mesh, "DG", 0)  # vizualization of stress

        while (t < T + 1e-14):
            traction = (CreepLoad/0.004)*(t if t <= 0.004 else 0.004)
            
            Dforce = Constant((0., 0., traction - traction_old))
            # problem.add_force_term(dot(TestFunction(problem.Vd), traction)*ds(1))
            problem.add_force_term(dot(TestFunction(problem.Vd), Dforce)*ds(1))

            n = FacetNormal(mesh)
            # print( assemble( (dot(Dforce, n) ) * ds(1) ) )
            ...
            top_displ.append(problem.u(0.,1.,1.)[2])
