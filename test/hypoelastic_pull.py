...
class PullTest(unittest.TestCase):
    def setUp(self):
        ...
        print(bcl.get_boundary_values())
        bcr = df.DirichletBC(V0.sub(0), expr_right, right)
        print(bcr.get_boundary_values())
        bcf = df.DirichletBC(V0.sub(1), expr_front, front)
        print(bcf.get_boundary_values())
        bcb = df.DirichletBC(V0.sub(2), expr_back, back)
...
class PullTestBC(unittest.TestCase):
    def setUp(self):
        ...
        expr_right.u = expr_right.u / t_end
        print("here 1")
        bcl = df.DirichletBC(V.sub(0), expr_left, left)
        print(bcl.get_boundary_values())
        bcr = df.DirichletBC(V.sub(0), expr_right, right)
        print(bcr.get_boundary_values())
