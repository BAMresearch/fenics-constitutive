...
template <Constraint TC> class JH2Debug: public RefLawInterface
{
public:
    ...
    JH2Debug(std::shared_ptr<JH2Parameters> parameters, int n)
        : RefLawInterface(n), _param(parameters)
    {
        ...
        _internal_vars[Q::DEPS] = Eigen::VectorXd::Zero(_n);
        _internal_vars[Q::YIELD] = Eigen::VectorXd::Zero(_n);
    ...
    void EvaluateIP(int i, std::vector<Eigen::Ref<Eigen::VectorXd>>& inputs, double del_t) override
    {
        ...
        _internal_vars[Q::DEPS](i) = d_eps_eq;
        ...
        _internal_vars[Q::YIELD](i) = Y_yield;
        if (s_tr_eq > Y_yield){
