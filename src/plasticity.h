...
public:
    ...
    std::pair<double, Eigen::VectorXd> Call(Eigen::VectorXd ss) const
    {
        ...
        else
        {
            ...
            if (se == 0)
            {
                return {se, Eigen::VectorXd::Zero(_q)};
            }
            else
            {
...
public:
    ...
    std::tuple<Eigen::VectorXd, Eigen::MatrixXd, double, Eigen::VectorXd>
    correct_stress(Eigen::VectorXd sig_tr, double k0, double tol = 1e-9, int max_iters = 20)
    {
        double f;
        Eigen::VectorXd m;
        Eigen::MatrixXd dm;
